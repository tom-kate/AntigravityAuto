...
createApp({
  setup(){
    ...
    // Quota refresh
    async function refreshAllQuotas(){if(quotaLoading.value)return;quotaLoading.value=true;const tasks=[];for(const f of cpaFiles.value){if(f.auth_index){const e=(f.email||f.account).toLowerCase();tasks.push({email:e,authIndex:f.auth_index});}}if(!tasks.length){quotaLoading.value=false;showToast('没有可查询额度的凭证',false);return}for(const t of tasks)quotaLoadingSet[t.email]=true;let ok=0,fail=0;const failedEmails=[];await Promise.all(tasks.map(async t=>{try{const r=await axios.get('/api/cpa-quota?auth_index='+encodeURIComponent(t.authIndex));if(r.data&&r.data.length){quotas[t.email]=r.data;ok++}else{fail++;failedEmails.push(t.email)}}catch(e){fail++;failedEmails.push(t.email)}finally{delete quotaLoadingSet[t.email]}}));for(const e of failedEmails){try{await axios.post('/api/mark-failed',{email:e,operation_status:'账户封禁'})}catch(ex){}}if(failedEmails.length)loadMain();quotaLoading.value=false;showToast('额度刷新: '+ok+'成功'+(fail?' / '+fail+'失败(已标记封禁)':''));}
    async function refreshSingleQuota(email){const e=email.toLowerCase();const f=getCPAFile(email);if(!f||!f.auth_index){showToast('该账号无凭证',false);return}quotaLoadingSet[e]=true;try{const r=await axios.get('/api/cpa-quota?auth_index='+encodeURIComponent(f.auth_index));if(r.data&&r.data.length){quotas[e]=r.data;showToast(email.split('@')[0]+' 额度已刷新')}else{showToast('额度刷新失败, 已标记账户封禁',false);try{await axios.post('/api/mark-failed',{email:e,operation_status:'账户封禁'})}catch(ex){}loadMain()}}catch(e2){showToast('额度刷新失败, 已标记账户封禁',false);try{await axios.post('/api/mark-failed',{email:e,operation_status:'账户封禁'})}catch(ex){}loadMain()}finally{delete quotaLoadingSet[e]}}
